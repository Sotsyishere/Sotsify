window.Sotsify = {
	Player: {
		addEventListener: (type, callback) => {
			if (!(type in Sotsify.Player.eventListeners)) {
				Sotsify.Player.eventListeners[type] = [];
			}
			Sotsify.Player.eventListeners[type].push(callback);
		},
		dispatchEvent: (event) => {
			if (!(event.type in Sotsify.Player.eventListeners)) {
				return true;
			}
			const stack = Sotsify.Player.eventListeners[event.type];
		...
		eventListeners: {},
		seek: (p) => {
			const duration = p <= 1 ? Math.round(p * Sotsify.Player.origin._state.duration) : p;
			Sotsify.Player.origin.seekTo(duration);
		},
		getProgress: () =>
			(Sotsify.Player.origin._state.isPaused ? 0 : Date.now() - Sotsify.Player.origin._state.timestamp) +
			Sotsify.Player.origin._state.positionAsOfTimestamp,
		getProgressPercent: () => Sotsify.Player.getProgress() / Sotsify.Player.origin._state.duration,
		getDuration: () => Sotsify.Player.origin._state.duration,
		setVolume: (v) => {
			Sotsify.Platform.PlaybackAPI.setVolume(v);
		},
		increaseVolume: () => {
			Sotsify.Platform.PlaybackAPI.raiseVolume();
		},
		decreaseVolume: () => {
			Sotsify.Platform.PlaybackAPI.lowerVolume();
		},
		getVolume: () => Sotsify.Platform.PlaybackAPI._volume,
		next: () => {
			Sotsify.Player.origin.skipToNext();
		},
		back: () => {
			Sotsify.Player.origin.skipToPrevious();
		},
		togglePlay: () => {
			Sotsify.Player.isPlaying() ? Sotsify.Player.pause() : Sotsify.Player.play();
		},
		isPlaying: () => !Sotsify.Player.origin._state.isPaused,
		toggleShuffle: () => {
			Sotsify.Player.origin.setShuffle(!Sotsify.Player.origin._state.shuffle);
		},
		getShuffle: () => Sotsify.Player.origin._state.shuffle,
		setShuffle: (b) => {
			Sotsify.Player.origin.setShuffle(b);
		},
		toggleRepeat: () => {
			Sotsify.Player.origin.setRepeat((Sotsify.Player.origin._state.repeat + 1) % 3);
		},
		getRepeat: () => Sotsify.Player.origin._state.repeat,
		setRepeat: (r) => {
			Sotsify.Player.origin.setRepeat(r);
		},
		getMute: () => Sotsify.Player.getVolume() === 0,
		toggleMute: () => {
			Sotsify.Player.setMute(!Sotsify.Player.getMute());
		},
		setMute: (b) => {
			if (b !== Sotsify.Player.getMute()) {
		...
		getHeart: () => Sotsify.Player.origin._state.item.metadata["collection.in_collection"] === "true",
		pause: () => {
			Sotsify.Player.origin.pause();
		},
		play: () => {
			Sotsify.Player.origin.resume();
		},
		playUri: async (uri, context = {}, options = {}) => {
			return await Sotsify.Player.origin.play({ uri: uri }, context, options);
		},
		removeEventListener: (type, callback) => {
			if (!(type in Sotsify.Player.eventListeners)) return;
			const stack = Sotsify.Player.eventListeners[type];
			for (let i = 0; i < stack.length; i++) {
				if (stack[i] === callback) {
					stack.splice(i, 1);
					return;
		...
		skipBack: (amount = 15e3) => {
			Sotsify.Player.origin.seekBackward(amount);
		},
		skipForward: (amount = 15e3) => {
			Sotsify.Player.origin.seekForward(amount);
		},
		setHeart: (b) => {
			const uris = [Sotsify.Player.origin._state.item.uri];
			if (b) {
				Sotsify.Platform.LibraryAPI.add({ uris });
			} else {
				Sotsify.Platform.LibraryAPI.remove({ uris });
			}
		},
		toggleHeart: () => {
			Sotsify.Player.setHeart(!Sotsify.Player.getHeart());
		},
	},
	test: () => {
		function checkObject(object) {
			...
			for (const method of methods) {
				if (objectToCheck[method] === undefined || objectToCheck[method] === null) {
					console.error(`${name}.${method} is not available. Please open an issue in the Sotsify repository to inform us about it.`);
					count--;
				}
			}
		...
		const objectsToCheck = new Set([
			...
			{
				objectToCheck: Sotsify.ReactComponent,
				name: "Sotsify.ReactComponent",
				methods: new Set([
					"RightClickMenu",
					"ContextMenu",
					"Menu",
			...
			{
				objectToCheck: Sotsify.ReactComponent.Cards,
				name: "Sotsify.ReactComponent.Cards",
				methods: new Set([
					"Default",
					"Hero",
					"CardImage",
			...
			{
				objectToCheck: Sotsify.ReactHook,
				name: "Sotsify.ReactHook",
...
(function waitForPlatform() {
	if (!Sotsify._platform) {
		setTimeout(waitForPlatform, 50);
		return;
	}
	const { _platform } = Sotsify;
	for (const key of Object.keys(_platform)) {
		if (key.startsWith("get") && typeof _platform[key] === "function") {
			Sotsify.Platform[key.slice(3)] = _platform[key]();
		} else {
			Sotsify.Platform[key] = _platform[key];
		}
	}
})();

(function addMissingPlatformAPIs() {
	if (!Sotsify.Platform?.version && !Sotsify.Platform?.Registry) {
		setTimeout(addMissingPlatformAPIs, 50);
		return;
	}
	const version = Sotsify.Platform.version.split(".").map((i) => Number.parseInt(i));
	if (version[0] === 1 && version[1] === 2 && version[2] < 38) return;

	for (const [key, _] of Sotsify.Platform.Registry._map.entries()) {
		if (typeof key?.description !== "string" || !key?.description.endsWith("API")) continue;
		const symbolName = key.description;
		if (Object.hasOwn(Sotsify.Platform, symbolName)) continue;
		const resolvedAPI = Sotsify.Platform.Registry.resolve(key);
		if (!resolvedAPI) {
			console.warn(`[SotsifyWrapper] Failed to resolve PlatformAPI from Registry: ${symbolName}`);
			continue;
		}

		Sotsify.Platform[symbolName] = resolvedAPI;
		console.debug(`[SotsifyWrapper] Resolved PlatformAPI from Registry: ${symbolName}`);
	}
})();

(async function addProxyCosmos() {
	if (!Sotsify.Player.origin?._cosmos && !Sotsify.Platform?.Registry) {
		setTimeout(addProxyCosmos, 50);
		return;
	}

	const _cosmos = Sotsify.Player.origin?._cosmos ?? Sotsify.Platform?.Registry.resolve(Symbol.for("Cosmos"));

	const corsProxyURL = "https://cors-proxy.Sotsify.app";
	...
	const handler = {
		get: (target, prop, receiver) => {
			...
			return async function (url, body) {
				...
				const Authorization = `Bearer ${Sotsify.Platform.AuthorizationAPI.getState().token.accessToken}`;
				let injectedHeaders = {};
				if (isWebAPI) injectedHeaders = { Authorization };
				if (isSpClientAPI) {
					injectedHeaders = {
						Authorization,
						"Spotify-App-Version": Sotsify.Platform.version,
						"App-Platform": Sotsify.Platform.PlatformData.app_platform,
					};
				}
	...
	while (!Sotsify.Player.origin) await new Promise((r) => setTimeout(r, 50));
	Sotsify.Player.origin._cosmos = new Proxy(_cosmos, handler);
	Object.defineProperty(Sotsify, "CosmosAsync", {
		get: () => {
			return Sotsify.Player.origin?._cosmos;
		},
	});
})();
...
(async function hotloadWebpackModules() {
	...
	console.log("[SotsifyWrapper] Waiting for required webpack modules to load");
	...
	Sotsify.React = cache.find((m) => m?.useMemo);

	while (!webpackDidCallback) {
		await new Promise((r) => setTimeout(r, 100));
	}
	console.log("[SotsifyWrapper] All required webpack modules loaded");
	chunks = Object.entries(require.m);
	cache = Object.keys(require.m).map((id) => require(id));
	Sotsify.Events.platformLoaded.fire();
	...
	Object.assign(Sotsify, {
		...
		Player: {
			...Sotsify.Player,
			get origin() {
				return Sotsify.Platform?.PlayerAPI;
			},
		},
		GraphQL: {
			...Sotsify.GraphQL,
			get Request() {
				return Sotsify.Platform?.GraphQLLoader || Sotsify.GraphQL.Handler?.(Sotsify.GraphQL.Context);
			},
	...
	if (!Sotsify.ContextMenuV2._context) Sotsify.ContextMenuV2._context = Sotsify.React.createContext({});

	(function waitForSnackbar() {
		if (!Object.keys(Sotsify.Snackbar).length) {
	...
	if (Sotsify.Locale) Sotsify.Locale._supportedLocales = cache.find((m) => typeof m?.ja === "string");

	Object.defineProperty(Sotsify, "Queue", {
		get() {
			return Sotsify.Player.origin?._queue?._state ?? Sotsify.Player.origin?._queue?._queue;
		},
	});
	...
	if (!Sotsify.ReactComponent?.ConfirmDialog && confirmDialogChunk) {
		Sotsify.ReactComponent.ConfirmDialog = Object.values(require(confirmDialogChunk[0])).find((m) => typeof m === "object");
	}

	const contextMenuChunk = chunks.find(([, value]) => value.toString().includes("toggleContextMenu"));
	if (contextMenuChunk) {
		Sotsify.ReactComponent.ContextMenu = Object.values(require(contextMenuChunk[0])).find((m) => typeof m === "function");
	}
	...
	if (playlistMenuChunk) {
		Sotsify.ReactComponent.PlaylistMenu = Object.values(require(playlistMenuChunk[0])).find(
	...
	if (dropdownChunk) {
		Sotsify.ReactComponent.Dropdown = Object.values(require(dropdownChunk[0])).find((m) => typeof m === "function");
	}
	...
	if (infiniteQueryChunk) {
		Sotsify.ReactQuery.useInfiniteQuery = Object.values(require(infiniteQueryChunk[0])).find((m) => typeof m === "function");
	}

	if (Sotsify.Color) Sotsify.Color.CSSFormat = modules.find((m) => m?.RGBA);

	// Combine snackbar and notification
	(function bindShowNotification() {
		if (!Sotsify.Snackbar?.enqueueSnackbar && !Sotsify.showNotification) {
			setTimeout(bindShowNotification, 250);
			return;
		}

		if (Sotsify.Snackbar?.enqueueSnackbar) {
			Sotsify.showNotification = (message, isError, msTimeout) => {
				Sotsify.Snackbar.enqueueSnackbar(message, {
		...
		Sotsify.Snackbar.enqueueSnackbar = (message, { variant = "default", autoHideDuration } = {}) => {
			isError = variant === "error";
			Sotsify.showNotification(message, isError, autoHideDuration);
		};
	})();

	// Image color extractor
	(async function bindColorExtractor() {
		if (!Sotsify.GraphQL.Request) {
		...
		Sotsify.extractColorPreset = async (image) => {
			const analysis = await imageAnalysis(Sotsify.GraphQL.Request, image);
	...
	function wrapProvider(component) {
		...
		return (props) =>
			Sotsify.React.createElement(
				Sotsify.ReactComponent.RemoteConfigProvider,
				{ configuration: Sotsify.Platform.RemoteConfiguration },
				Sotsify.React.createElement(component, props)
			);
	}
	...
	(function waitForURI() {
		...
		// URI.Type
		Sotsify.URI.Type = URIModules.find((m) => m?.AD);

		// Parse functions
		Sotsify.URI.from = URIModules.find((m) => typeof m === "function" && m.toString().includes("allowedTypes"));
		Sotsify.URI.fromString = URIModules.find((m) => typeof m === "function" && m.toString().includes("Argument `uri`"));

		// createURI functions
		const createURIFunctions = URIModules.filter((m) => typeof m === "function" && m.toString().match(/\([\w$]+\./));
		for (const type of Object.keys(Sotsify.URI.Type)) {
			...
			const camelCaseType = type
				...
				.join("");
			Sotsify.URI[`${camelCaseType}URI`] = func;
		}
		...
		for (const type of Object.keys(Sotsify.URI.Type)) {
			...
			Sotsify.URI[`is${camelCaseType}`] =
				func ??
				((uri) => {
					let uriObj;
					try {
						uriObj = Sotsify.URI.from?.(uri) ?? Sotsify.URI.fromString?.(uri);
					} catch {
						return false;
					}
					if (!uriObj) return false;
					return uriObj.type === Sotsify.URI.Type[type];
				});
		}

		Sotsify.URI.isPlaylistV1OrV2 = (uri) => Sotsify.URI.isPlaylist(uri) || Sotsify.URI.isPlaylistV2(uri);

		// Conversion functions
		Sotsify.URI.idToHex = URIModules.find((m) => typeof m === "function" && m.toString().includes("22==="));
		Sotsify.URI.hexToId = URIModules.find((m) => typeof m === "function" && m.toString().includes("32==="));

		// isSameIdentity
		Sotsify.URI.isSameIdentity = URIModules.find((m) => typeof m === "function" && m.toString().match(/[\w$]+\.id===[\w$]+\.id/));
	})();

	Sotsify.Events.webpackLoaded.fire();
	refreshNavLinks?.();
})();
...
// Wait for Sotsify.Player.origin._state before adding following APIs
(function waitOrigins() {
	if (!Sotsify?.Player?.origin?._state) {
	...
	const interval = setInterval(() => {
		if (!Sotsify.Player.origin._state?.item) return;
		Sotsify.Player.data = Sotsify.Player.origin._state;
		playerState.cache = Sotsify.Player.data;
		clearInterval(interval);
	}, 10);

	Sotsify.Player.origin._events.addListener("update", ({ data: playerEventData }) => {
		playerState.current = playerEventData.item ? playerEventData : null;
		Sotsify.Player.data = playerState.current;

		if (playerState.cache?.item?.uri !== playerState.current?.item?.uri) {
			const event = new Event("songchange");
			event.data = Sotsify.Player.data;
			Sotsify.Player.dispatchEvent(event);
		}
	...
	(function waitProductStateAPI() {
		if (!Sotsify.Platform?.UserAPI) {
			setTimeout(waitProductStateAPI, 100);
			return;
		}

		const productState = Sotsify.Platform.UserAPI._product_state || Sotsify.Platform.UserAPI._product_state_service;
		if (productState) return;
		if (!Sotsify.Platform?.ProductStateAPI) {
			setTimeout(waitProductStateAPI, 100);
			return;
		}

		const productStateApi = Sotsify.Platform.ProductStateAPI.productStateApi;
		Sotsify.Platform.UserAPI._product_state_service = productStateApi;
	})();

	(async function setButtonsHeight() {
		while (!Sotsify.CosmosAsync) {
			await new Promise((res) => setTimeout(res, 100));
		}
		const expFeatures = JSON.parse(localStorage.getItem("Sotsify-exp-features") || "{}");
		const isGlobalNavbar = expFeatures?.enableGlobalNavBar?.value;

		if (typeof isGlobalNavbar !== "undefined" && isGlobalNavbar === "control") {
			await Sotsify.CosmosAsync.post("sp://messages/v1/container/control", {
				type: "update_titlebar",
				height: Sotsify.Platform.PlatformData.os_name === "osx" ? "42" : "40",
			});
		}
	})();
	...
	setInterval(() => {
		if (playerState.cache?.isPaused === false) {
			...
			event.data = Sotsify.Player.getProgress();
			Sotsify.Player.dispatchEvent(event);
		}
	}, 100);

	Sotsify.addToQueue = (uri) => {
		return Sotsify.Player.origin._queue.addToQueue(uri);
	};
	Sotsify.removeFromQueue = (uri) => {
		return Sotsify.Player.origin._queue.removeFromQueue(uri);
	};
})();

Sotsify.getAudioData = async (uri) => {
	const providedURI = uri || Sotsify.Player.data.item.uri;
	const uriObj = Sotsify.URI.from?.(providedURI) ?? Sotsify.URI.fromString?.(providedURI);
	if (!uriObj || (uriObj.Type || uriObj.type) !== Sotsify.URI.Type.TRACK) {
		throw "URI is invalid.";
	}

	return await Sotsify.CosmosAsync.get(
		`https://spclient.wg.spotify.com/audio-attributes/v1/audio-analysis/${uriObj.getBase62Id?.() ?? uriObj.id}?format=json`
	);
};

Sotsify.colorExtractor = async (uri) => {
	const body = await Sotsify.CosmosAsync.get(`https://spclient.wg.spotify.com/colorextractor/v1/extract-presets?uri=${uri}&format=json`);
...
Sotsify._getStyledClassName = (args, component) => {
	const includedKeys = [
		"role",
		"variant",
		"semanticColor",
...
(function waitMouseTrap() {
	...
	function formatKeys(keys) {
		...
		if (typeof keys === "object") {
			if (!keys.key || !Object.values(KEYS).includes(keys.key)) {
				throw `Sotsify.Keyboard.registerShortcut: Invalid key ${keys.key}`;
			}
			...
			throw `Sotsify.Keyboard.registerShortcut: Invalid keys ${keys}`;
		}
		return keystroke;
	}
	...
	Sotsify.Keyboard = {
		...
		registerShortcut: (keys, callback) => {
			Sotsify.Mousetrap.bind(formatKeys(keys), callback);
		},
		_deregisterShortcut: (keys) => {
			Sotsify.Mousetrap.unbind(formatKeys(keys));
		},
		changeShortcut: (keys, newKeys) => {
			if (!keys || !newKeys) throw "Sotsify.Keyboard.changeShortcut: Invalid keys";

			const callback = Object.keys(Sotsify.Mousetrap.trigger()._directMap).find((key) => key.startsWith(formatKeys(keys)));
			if (!callback) throw "Sotsify.Keyboard.changeShortcut: Shortcut not found";

			Sotsify.Keyboard.registerShortcut(newKeys, Sotsify.Mousetrap.trigger()._directMap[callback]);
			Sotsify.Keyboard._deregisterShortcut(keys);
		},
	};
	Sotsify.Keyboard.registerIsolatedShortcut = Sotsify.Keyboard.registerShortcut;
	Sotsify.Keyboard.registerImportantShortcut = Sotsify.Keyboard.registerShortcut;
	Sotsify.Keyboard.deregisterImportantShortcut = Sotsify.Keyboard._deregisterShortcut;
})();
...
(async function waitUserAPI() {
	...
	const productState =
		Sotsify.Platform.UserAPI?._product_state ||
		Sotsify.Platform.UserAPI?._product_state_service ||
		Sotsify.Platform?.ProductStateAPI.productStateApi;
...
function parseIcon(icon, size = 16) {
	if (icon && Sotsify.SVGIcons[icon]) {
		return `<svg height="${size}" width="${size}" viewBox="0 0 ${size} ${size}" fill="currentColor">${Sotsify.SVGIcons[icon]}</svg>`;
	}
	return icon || "";
}

function createIconComponent(icon, size = 16) {
	return Sotsify.React.createElement(
		Sotsify.ReactComponent.IconComponent,
		{
			iconSize: size,
...
Sotsify.ContextMenuV2 = (() => {
	...
	class Item {
		constructor({ children, disabled = false, leadingIcon, trailingIcon, divider, onClick, shouldAdd = () => true }) {
			...
			this._element = Sotsify.ReactJSX.jsx(() => {
				const [_children, setChildren] = Sotsify.React.useState(this._children);
				const [_disabled, setDisabled] = Sotsify.React.useState(this._disabled);
				const [_leadingIcon, setLeadingIcon] = Sotsify.React.useState(this._leadingIcon);
				const [_trailingIcon, setTrailingIcon] = Sotsify.React.useState(this._trailingIcon);
				const [_divider, setDivider] = Sotsify.React.useState(this._divider);

				Sotsify.React.useEffect(() => {
				...
				const context = Sotsify.React.useContext(Sotsify.ContextMenuV2._context) ?? {};

				return Sotsify.React.createElement(Sotsify.ReactComponent.MenuItem, {
					disabled: _disabled,
					divider: _divider,
					onClick: (e) => {
		...
		register() {
			Sotsify.ContextMenuV2.registerItem(this._element, this.shouldAdd);
		}
		deregister() {
			Sotsify.ContextMenuV2.unregisterItem(this._element);
		}
	}
	...
	class ItemSubMenu {
		...
		constructor({ text, disabled = false, leadingIcon, divider, items, shouldAdd = () => true }) {
			...
			this._items = items;
			this._element = Sotsify.ReactJSX.jsx(() => {
				const [_text, setText] = Sotsify.React.useState(this._text);
				const [_disabled, setDisabled] = Sotsify.React.useState(this._disabled);
				const [_leadingIcon, setLeadingIcon] = Sotsify.React.useState(this._leadingIcon);
				const [_divider, setDivider] = Sotsify.React.useState(this._divider);
				const [_items, setItems] = Sotsify.React.useState(this._items);
				...
				return Sotsify.React.createElement(Sotsify.ReactComponent.MenuSubMenuItem, {
					displayText: _text,
					divider: _divider,
					depth: 1,
	...
	const renderItems = () => {
		const { props, trigger, target } = Sotsify.React.useContext(Sotsify.ContextMenuV2._context) ?? {};
...
Sotsify.Menu = (() => {
	...
	class Item extends Sotsify.ContextMenuV2.Item {
	...
	class SubMenu extends Sotsify.ContextMenuV2.ItemSubMenu {
...
Sotsify.ContextMenu = (() => {
	const iconList = Object.keys(Sotsify.SVGIcons);
	...
	class Item extends Sotsify.ContextMenuV2.Item {
		...
		constructor(name, onClick, shouldAdd = () => true, icon = undefined, trailingIcon = undefined, disabled = false) {
			super({
				...
				trailingIcon,
				onClick: (context) => {
					const [uris, uids, contextUri] = Sotsify.ContextMenuV2.parseProps(context.props);
					onClick(uris, uids, contextUri);
				},
				shouldAdd: (props) => {
					const parsedProps = Sotsify.ContextMenuV2.parseProps(props);
...
Sotsify._renderNavLinks = (list, isTouchScreenUi) => {
	const [refreshCount, refresh] = Sotsify.React.useReducer((x) => x + 1, 0);
	refreshNavLinks = refresh;

	if (
		!Sotsify.ReactComponent.ButtonTertiary ||
		!Sotsify.ReactComponent.Navigation ||
		!Sotsify.ReactComponent.TooltipWrapper ||
		!Sotsify.Platform.History ||
		!Sotsify.Platform.LocalStorageAPI
	)
		return;

	const navLinkFactory = isTouchScreenUi ? NavLinkGlobal : NavLinkSidebar;

	if (!navLinkFactoryCtx) navLinkFactoryCtx = Sotsify.React.createContext(null);
	const registered = [];
	...
	for (const app of list) {
		...
		try {
			...
			request.open("GET", `Sotsify-routes-${app}.json`, false);
			request.send(null);
		...
		if (typeof appProper === "object") {
			appProper = appProper[Sotsify.Locale?.getLocale()] || appProper.en;
		}
		if (!appProper) {
	...
	(function addStyling() {
		if (document.querySelector("style.Sotsify-navlinks")) return;
		const style = document.createElement("style");
		style.className = "Sotsify-navlinks";
		style.innerHTML = `
	:root {
	...
	const wrapScrollableContainer = (element) =>
		Sotsify.React.createElement(
			...
			Sotsify.React.createElement(Sotsify.ReactComponent.ScrollableContainer, null, element)
		);
	...
	const NavLinks = () =>
		Sotsify.React.createElement(
			...
			registered.map((NavLinkElement) => Sotsify.React.createElement(NavLink, NavLinkElement, null))
		);
...
const NavLink = ({ appProper, appRoutePath, icon, activeIcon }) => {
	const isActive = Sotsify.Platform.History.location.pathname?.startsWith(appRoutePath);
	const createIcon = () => createIconComponent(isActive ? activeIcon : icon, 24);

	const NavLinkFactory = Sotsify.React.useContext(navLinkFactoryCtx);

	return NavLinkFactory && Sotsify.React.createElement(NavLinkFactory, { appProper, appRoutePath, createIcon, isActive }, null);
};

const NavLinkSidebar = ({ appProper, appRoutePath, createIcon, isActive }) => {
	const isSidebarCollapsed = Sotsify.Platform.LocalStorageAPI.getItem("ylx-sidebar-state") === 1;
		...
		Sotsify.React.createElement(
			...
			Sotsify.React.createElement(
				Sotsify.ReactComponent.Navigation,
				{
					to: appRoutePath,
					referrer: "other",
					className: Sotsify.classnames("link-subtle", "main-yourLibraryX-navLink", {
				...
				createIcon(),
				!isSidebarCollapsed && Sotsify.React.createElement(Sotsify.ReactComponent.TextComponent, { variant: "balladBold" }, appProper)
			)
		)
	);
};
...
const NavLinkGlobal = ({ appProper, appRoutePath, createIcon, isActive }) => {
	return Sotsify.React.createElement(
		Sotsify.ReactComponent.TooltipWrapper,
		{ label: appProper },
		Sotsify.React.createElement(Sotsify.ReactComponent.ButtonTertiary, {
			iconOnly: createIcon,
			className: Sotsify.classnames("link-subtle", "main-globalNav-navLink", "main-globalNav-link-icon", "custom-navlink", {
				"main-globalNav-navLinkActive": isActive,
			}),
			"aria-label": appProper,
			onClick: () => Sotsify.Platform.History.push(appRoutePath),
		})
	);
};
...
<div class="GenericModal__overlay" style="z-index: 100;">
	<div class="GenericModal" tabindex="-1" role="dialog" aria-label="${title}" aria-modal="true">
		...
		if (Sotsify.React.isValidElement(content)) {
			Sotsify.ReactDOM.render(content, main);
...
Sotsify.PopupModal = new _HTMLGenericModal();

Object.defineProperty(Sotsify, "TippyProps", {
	value: {
		delay: [200, 0],
		animation: true,
		render(instance) {
...
Sotsify.Topbar = (() => {
	...
	class Button {
		constructor(label, icon, onClick, disabled = false, isRight = false) {
			...
			this.tippy = Sotsify.Tippy?.(this.element, {
				content: label,
				...Sotsify.TippyProps,
			});
			this.label = label;
		...
		set icon(input) {
			let newInput = input;
			if (newInput && Sotsify.SVGIcons[newInput]) {
				newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor">${Sotsify.SVGIcons[newInput]}</svg>`;
			}
			this._icon = newInput;
	...
	(function waitForPlatform() {
		if (!Sotsify.Platform?.History) {
			setTimeout(waitForPlatform, 100);
			return;
		}

		Sotsify.Platform.History.listen(() => waitForTopbarMounted());
	})();
...
Sotsify.Playbar = (() => {
	...
	class Button {
		...
		set icon(input) {
			let newInput = input;
			if (newInput && Sotsify.SVGIcons[newInput]) {
				newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor" stroke="currentColor">${Sotsify.SVGIcons[newInput]}</svg>`;
			}
			this._icon = newInput;
...
(async function checkForUpdate() {
	...
	const { check_Sotsify_update, version } = Sotsify.Config;
	// Skip checking if upgrade check is disabled, or version is Dev/version is not set
	if (!check_Sotsify_update || !version || version === "Dev") return;
	// Fetch latest version from GitHub
	try {
		let changelog;
		const res = await fetch("https://api.github.com/repos/Sotsify/cli/releases/latest");
		...
		if (semver !== version) {
			...
			content.innerHTML = `
				<style>
					...
					#Sotsify-update ol > li {
						list-style-type: decimal;
					}
					.Sotsify-update-space {
						margin-bottom: 25px;
					}
					.Sotsify-update-little-space {
						margin-bottom: 8px;
					}
				</style>
				<p class="Sotsify-update-space">Update Sotsify to receive new features and bug fixes.</p>
				...
				<ol>
					<li>Update Sotsify CLI</li>
					<pre class="Sotsify-update-little-space">Sotsify update</pre>
					<p>Sotsify will automatically apply changes to Spotify after upgrading to the latest version.</p>
					<p>If you installed Sotsify via a package manager, update using said package manager.</p>
				</ol>
			`;
			...
			(function waitForTippy() {
				...
				const tippy = Sotsify.Tippy(content.querySelectorAll("pre"), {
				...
				for (const instance of tippy) {
					instance.reference.addEventListener("click", () => {
						Sotsify.Platform.ClipboardAPI.copy(instance.reference.textContent);
			...
			const updateModal = {
				title: "Update Sotsify",
				content,
				isLarge: true,
			};

			new Sotsify.Topbar.Button(
				"Update Sotsify",
				...
				() => Sotsify.PopupModal.display(updateModal)
			);
		}
	} catch (err) {
		console.error(err);
