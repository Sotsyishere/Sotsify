(async () => {
	...
	try {
		overrideList = JSON.parse(localStorage.getItem("Sotsify-exp-features"));
	...
	Sotsify.expFeatureOverride = (feature) => {
		hooksPatched = true;
		...
		localStorage.setItem("Sotsify-exp-features", JSON.stringify(overrideList));
		return feature;
	};

	const content = document.createElement("div");
	content.classList.add("Sotsify-exp-features");
	const style = document.createElement("style");
	style.innerHTML = `
.Sotsify-exp-features .col {
    padding: 0;
}
.Sotsify-exp-features .setting-row::after {
    content: "";
    display: table;
    clear: both;
}
.Sotsify-exp-features .setting-row {
    display: flex;
    padding: 10px 0;
    align-items: center;
}
.Sotsify-exp-features .setting-row .col.description {
    float: left;
    padding-right: 15px;
    width: 100%;
}
.Sotsify-exp-features .setting-row .col.action {
    float: right;
    text-align: right;
}
.Sotsify-exp-features .setting-row .col.action .dropdown {
	width: max-content;
}
.Sotsify-exp-features button.switch {
    align-items: center;
    border: 0px;
    border-radius: 50%;
...
.Sotsify-exp-features button.switch.disabled,
.Sotsify-exp-features button.switch[disabled] {
    color: rgba(var(--spice-rgb-text), .3);
}
.Sotsify-exp-features button.reset {
	font-weight: 700;
	font-size: medium;
...
.Sotsify-exp-features button.reset:hover {
	transform: scale(1.04);
	border-color: var(--spice-text);
}
.Sotsify-exp-features .search-container {
    width: 100%;
}
.Sotsify-exp-features .setting-row#search .col.action {
    position: relative;
    width: 100%;
}
.Sotsify-exp-features .setting-row#search svg {
    position: absolute;
    margin: 12px;
}
.Sotsify-exp-features input.search {
    border-style: solid;
	...
	(function waitForRemoteConfigResolver() {
		...
		if (!hooksPatched || (!Sotsify.RemoteConfigResolver && !Sotsify.Platform?.RemoteConfiguration)) {
			setTimeout(waitForRemoteConfigResolver, 500);
			return;
		}

		let remoteConfiguration = Sotsify.RemoteConfigResolver?.value.remoteConfiguration || Sotsify.Platform?.RemoteConfiguration;
		let setOverrides = () => {};

		(function waitForResolver() {
			if (!Sotsify.RemoteConfigResolver) {
				isFallback = true;
			...
			isFallback = false;
			notice.remove();
			remoteConfiguration = Sotsify.RemoteConfigResolver.value.remoteConfiguration;
			setOverrides = Sotsify.RemoteConfigResolver.value.setOverrides;
		})();

		for (const key of Object.keys(overrideList)) {
			if (newFeatures.includes(key)) continue;
			delete overrideList[key];
			console.warn(`[Sotsify-exp-features] Removed ${key} from override list`);
			localStorage.setItem("Sotsify-exp-features", JSON.stringify(overrideList));
		}
		...
		function changeValue(name, value) {
			...
			setOverrides(Sotsify.createInternalMap?.(featureMap));
		}
...
<input type="text" class="search" placeholder="Search for a feature">
		...
		resetBtn.onclick = () => {
			localStorage.removeItem("Sotsify-exp-features");
