...
func main() {
		...
		path, err := (func() (string, error) {
			if styleFocus {
				...
				if len(commands) == 0 && len(flags) == 0 {
					...
					return utils.GetSotsifyFolder(), nil
				}
	...
	utils.PrintBold("Sotsify v" + version)
	if slices.Contains(commands, "upgrade") || slices.Contains(commands, "update") {
		...
		spotStat := spotifystatus.Get(spotifyPath)
		if spotStat.IsBackupable() {
			utils.PrintNote("Sotsify is already up-to-date! If you ran this command because Sotsify disappeared after Spotify updated, we'll attempt to fix it for you right now.")
			cmd.Backup(version)
			cmd.CheckStates()
			cmd.InitSetting()
			cmd.Apply(version)
	...
	for _, v := range commands {
		...
		default:
			...
			utils.PrintInfo(`Run "Sotsify -h" for list of valid commands.`)
			os.Exit(1)
		}
	}
}
...
func help() {
	...
	log.Println(utils.Bold("USAGE") + "\n" +
		"Sotsify [-q] [-e] [-a] \x1B[4mcommand\033[0m...\n" +
		"Sotsify {-c | --config} | {-v | --version} | {-h | --help}\n\n" +
...
path                Prints path of Spotify's executable, userdata, and more.
                    ...
                    Sotsify path userdata

                    3. Print all paths:
                    Sotsify path all

                    4. Toggle focus with flags:
                    Sotsify path <flag> <option>
...
config              1. Print all config fields and values:
                    ...
                    Sotsify config <field>

                    Example usage:
                    Sotsify config color_scheme
                    Sotsify config custom_apps

                    3. Change value of one or multiple config fields.
                    Sotsify config <field> <value> [<field> <value> ...]
                    ...
                    Sotsify config disable_sentry 1
                    - Add extension "myFakeExt.js" to current extensions list:
                    Sotsify config extensions myFakeExt.js
                    - Remove extension "wrongname.js" from extensions list:
                    Sotsify config extensions wrongname.js-
                    - Disable "inject_css" and enable "song_page"
                    Sotsify config inject_css 0 song_page 1
...
For config information, run "Sotsify -h config".
For more information and bug report: https://github.com/Sotsify/cli/`)
}
...
spotify_launch_flags
    ...
    List of valid flags: https://Sotsify.app/docs/development/spotify-cli-flags

always_enable_devtools <0 | 1>
    Whether Chrome DevTools is enabled when launching/restarting Spotify.

check_Sotsify_update <0 | 1>
		Whether to check for CLI update when running Sotsify.
...
expose_apis <0 | 1>
    Leaks some Spotify's API, functions, objects to Sotsify global object that
