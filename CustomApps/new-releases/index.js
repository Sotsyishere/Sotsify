...
let dismissed;
try {
	dismissed = JSON.parse(Sotsify.LocalStorage.get("new-releases:dismissed"));
...
class Grid extends react.Component {
	...
	removeCards(id, type) {
		switch (type) {
			case "reset":
				Sotsify.showNotification("Reset dismissed releases");
				dismissed = [];
				break;
			case "undo":
				if (!dismissed[0]) Sotsify.showNotification("Nothing to undo", true);
				else Sotsify.showNotification("Undone dismissal");
		...
		Sotsify.LocalStorage.set("new-releases:dismissed", JSON.stringify(dismissed));
	...
	async componentDidMount() {
		...
		this.configButton = new Sotsify.Menu.Item(
			"New Releases config",
			false,
			openConfig,
	...
	render() {
		...
		return react.createElement(
			...
			react.createElement(
				...
				react.createElement("h1", null, Sotsify.Locale.get("new_releases")),
				react.createElement(
					"div",
					{
					...
					react.createElement(ButtonText, {
						text: Sotsify.Locale.get("playlist.extender.refresh"),
...
async function getArtistList() {
	...
	const artists = await Sotsify.Platform.LibraryAPI.getContents(config);
	count(true);
	return artists.items ?? [];
}

async function getArtistEverything(artist) {
	const { data, errors } = await Sotsify.GraphQL.Request(
		{
	...
	const items = [];
	const types = [
		[CONFIG.album, releases.filter((r) => r.type === "ALBUM"), Sotsify.Locale.get("album")],
		...
		// [CONFIG["appears-on"], releases.appears_on?.releases, Sotsify.Locale.get("artist.appears-on")],
		[CONFIG.compilations, releases.filter((r) => r.type === "COMPILATION"), Sotsify.Locale.get("compilation")],
		[
			CONFIG["single-ep"],
			releases.filter((r) => r.type === "SINGLE" || r.type === "EP"),
			`${Sotsify.Locale.get("single")}/${Sotsify.Locale.get("ep")}`,
		],
	];
...
async function getPodcastList() {
	const body = await Sotsify.Platform.LibraryAPI.getShows({ limit: 50000 });
	return body.items ?? [];
}

async function getPodcastRelease(uri) {
	const body = await Sotsify.Platform.ShowAPI.getContents(uri, { limit: 50000 });
	return body.items;
}
...
async function fetchTracks() {
	...
	Sotsify.showNotification(`Fetching releases from ${artistList.length} artists`);
...
async function fetchPodcasts() {
	const items = [];
	const itemTypeStr = Sotsify.Locale.get("card.tag.episode");
