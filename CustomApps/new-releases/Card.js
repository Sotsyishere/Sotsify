function DraggableComponent({ uri, title, children }) {
	...
	.ReactHook.DragHandler?.([uri], title);
	return dragHandler
...
class Card extends react.Component {
	constructor(props) {
		...
		const uriType = Sotsify.URI.fromString(this.uri)?.type;
		switch (uriType) {
			case Sotsify.URI.Type.ALBUM:
			case Sotsify.URI.Type.TRACK:
				this.menuType = Sotsify.ReactComponent.AlbumMenu;
				break;
		}
	...
	play(event) {
		Sotsify.Player.playUri(this.uri, this.context);
	...
	closeButtonClicked(event) {
		...
		Sotsify.Snackbar.enqueueCustomSnackbar
			? Sotsify.Snackbar.enqueueCustomSnackbar("dismissed-release", {
					...
					children: Sotsify.ReactComponent.Snackbar.wrapper({
						children: Sotsify.ReactComponent.Snackbar.simpleLayout({
							leading: Sotsify.ReactComponent.Snackbar.styledImage({
							...
							trailing: Sotsify.ReactComponent.Snackbar.ctaText({
								ctaText: "Undo",
			...
			: Sotsify.showNotification(`Dismissed <b>${this.title}</b> from <br>${this.artist.name}</b>`);
	}
	...
	render() {
		...
		if (this.visual.count && this.trackCount) {
			detail.push(Sotsify.Locale.get("tracklist-header.songs-counter", this.trackCount));
		}

		return react.createElement(
			Sotsify.ReactComponent.RightClickMenu || "div",
			{
							...
							react.createElement(
								...
								react.createElement(
									"div",
									{
										...
										"aria-label": Sotsify.Locale.get("play"),
										style: { "--size": "40px" },
