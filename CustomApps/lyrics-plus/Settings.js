...
const RefreshTokenButton = ({ setTokenCallback }) => {
	...
	useEffect(() => {
		if (buttonText === "Refreshing token...") {
			Sotsify.CosmosAsync.get("https://apic-desktop.musixmatch.com/ws/1.1/token.get?app_id=web-desktop-app-v1.0", null, {
...
const ConfigHotkey = ({ name, defaultValue, onChange = () => {} }) => {
	const [value, setValue] = useState(defaultValue);
	const [trap] = useState(new Sotsify.Mousetrap());
	...
	return react.createElement(
		...
		react.createElement(
			...
			react.createElement(
				...
				react.createElement(SwapButton, {
					icon: Sotsify.SVGIcons["chart-up"],
					onClick: () => onSwap(item.name, -1),
					disabled: isFirst,
				}),
				react.createElement(SwapButton, {
					icon: Sotsify.SVGIcons["chart-down"],
					onClick: () => onSwap(item.name, 1),
					disabled: isLast,
				}),
				react.createElement(ButtonSVG, {
