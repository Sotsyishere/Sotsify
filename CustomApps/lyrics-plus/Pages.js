const CreditFooter = react.memo(({ provider, copyright }) => {
	...
	const credit = [Sotsify.Locale.get("web-player.lyrics.providedBy", provider)];
	if (copyright) {
...
const SyncedLyricsPage = react.memo(({ lyrics = [], provider, copyright, isKara }) => {
	...
	useTrackPosition(() => {
		const newPos = Sotsify.Player.getProgress();
			...
			activeLines.map(({ text, lineNumber, startTime }, i) => {
				...
				return react.createElement(
					"p",
					{
						...
						onClick: (event) => {
							if (startTime) {
								Sotsify.Player.seek(startTime);
							}
						},
						onContextMenu: (event) => {
							event.preventDefault();
							Sotsify.Platform.ClipboardAPI.copy(rawLyrics)
								.then(() => Sotsify.showNotification("Lyrics copied to clipboard"))
								.catch(() => Sotsify.showNotification("Failed to copy lyrics to clipboard"));
						},
					},
...
class SearchBar extends react.Component {
	...
	componentDidMount() {
		...
		this.toggleCallback = () => {
			if (!(Sotsify.Platform.History.location.pathname === "/lyrics-plus" && this.container)) return;
		...
		Sotsify.Mousetrap().bind("mod+shift+f", this.toggleCallback);
		Sotsify.Mousetrap(this.container).bind("mod+shift+f", this.toggleCallback);
		Sotsify.Mousetrap(this.container).bind("enter", this.loopThroughCallback);
		Sotsify.Mousetrap(this.container).bind("shift+enter", this.loopThroughCallback);
		Sotsify.Mousetrap(this.container).bind("esc", this.unFocusCallback);
	}

	componentWillUnmount() {
		Sotsify.Mousetrap().unbind("mod+shift+f", this.toggleCallback);
		Sotsify.Mousetrap(this.container).unbind("mod+shift+f", this.toggleCallback);
		Sotsify.Mousetrap(this.container).unbind("enter", this.loopThroughCallback);
		Sotsify.Mousetrap(this.container).unbind("shift+enter", this.loopThroughCallback);
		Sotsify.Mousetrap(this.container).unbind("esc", this.unFocusCallback);
	}
		...
		return react.createElement(
			...
			react.createElement("svg", {
				...
				dangerouslySetInnerHTML: {
					__html: Sotsify.SVGIcons.search,
				},
			}),
			react.createElement(
				"span",
...
const SyncedExpandedLyricsPage = react.memo(({ lyrics, provider, copyright, isKara }) => {
	...
	useTrackPosition(() => {
		if (!Sotsify.Player.data.is_paused) {
			setPosition(Sotsify.Player.getProgress() + CONFIG.visual["global-delay"] + CONFIG.visual.delay);
		}
	});
		...
		const lyricsEl1 = react.createElement(
			...
			react.createElement("div", {
				...
				onContextMenu: (event) => {
					...
					Sotsify.Platform.ClipboardAPI.copy(copylyrics)
						.then(() => Sotsify.showNotification("Lyrics copied to clipboard"))
						.catch(() => Sotsify.showNotification("Failed to copy lyrics to clipboard"));
				},
			})
