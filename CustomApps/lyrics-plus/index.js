...
/** @type {React} */
const react = Sotsify.React;
const { useState, useEffect, useCallback, useMemo, useRef } = react;
/** @type {import("react").ReactDOM} */
const reactDOM = Sotsify.ReactDOM;
const spotifyVersion = Sotsify.Platform.version;
...
class LyricsContainer extends react.Component {
	...
	async fetchColors(uri) {
		...
		try {
			try {
				const { fetchExtractedColorForTrackEntity } = Sotsify.GraphQL.Definitions;
				const { data } = await Sotsify.GraphQL.Request(fetchExtractedColorForTrackEntity, { uri });
				const { hex } = data.trackUnion.albumOfTrack.coverArt.extractedColors.colorDark;
				vibrant = Number.parseInt(hex.replace("#", ""), 16);
			} catch {
				const colors = await Sotsify.CosmosAsync.get(`https://spclient.wg.spotify.com/colorextractor/v1/extract-presets?uri=${uri}&format=json`);
	...
	async fetchTempo(uri) {
		const audio = await Sotsify.CosmosAsync.get(`https://api.spotify.com/v1/audio-features/${uri.split(":")[2]}`);
	...
	async translateLyrics(silent = true) {
		function showNotification(timeout) {
			if (silent) return;
			Sotsify.showNotification("Translating...", false, timeout);
		}
	...
	resetDelay() {
		CONFIG.visual.delay = Number(localStorage.getItem(`lyrics-delay:${Sotsify.Player.data.item.uri}`)) || 0;
	}
	...
	processLyricsFromFile(event) {
		...
		if (file[0].size > 1024 * 1024) {
			Sotsify.showNotification("File too large", true);
			return;
		}
		...
		reader.onload = (e) => {
			try {
				...
				if (!parsedKeys.length) {
					Sotsify.showNotification("Nothing to load", true);
					return;
				}
				...
				Sotsify.showNotification(`Loaded ${parsedKeys.join(", ")} lyrics from file`);
			} catch (e) {
				console.error(e);
				Sotsify.showNotification("Failed to load lyrics", true);
			}
		};

		reader.onerror = (e) => {
			console.error(e);
			Sotsify.showNotification("Failed to read file", true);
		};
	...
	componentDidMount() {
		...
		if (Sotsify.Player?.data?.item) {
			this.state.explicitMode = this.state.lockMode;
			this.currentTrackUri = Sotsify.Player.data.item.uri;
			this.fetchLyrics(Sotsify.Player.data.item, this.state.explicitMode);
		}
		...
		this.configButton = new Sotsify.Menu.Item("Lyrics Plus config", false, openConfig, "lyrics");
					...
					react.createElement(
						"button",
						{
							...
							onClick: () => {
								...
								if (!synced && !unsynced && !karaoke && !genius) {
									Sotsify.showNotification("No lyrics to cache", true);
									return;
								}

								this.saveLocalLyrics(this.currentTrackUri, { synced, unsynced, karaoke, genius });
								Sotsify.showNotification("Lyrics cached");
							},
						},
						react.createElement("svg", {
							...
							dangerouslySetInnerHTML: {
								__html: Sotsify.SVGIcons[this.state.isCached ? "downloaded" : "download"],
							},
						})
					)
				),
				react.createElement(
					...
					react.createElement(
						...
						react.createElement("svg", {
							...
							fill: "currentColor",
							dangerouslySetInnerHTML: {
								__html: Sotsify.SVGIcons["plus-alt"],
							},
						})
					)
				)
				...
				react.createElement(TopBarContent, {
					...
					switchCallback: (label) => {
						...
						if (mode !== this.state.mode) {
							...
							this.state.provider !== "local" && this.fetchLyrics(Sotsify.Player.data.item, mode);
						}
					},
					lockCallback: (label) => {
						...
						this.fetchLyrics(Sotsify.Player.data.item, mode);
						CONFIG.locked = mode;
