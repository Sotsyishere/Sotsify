...
var (
	SotsifyFolder         = utils.GetSotsifyFolder()
...
// tries to auto-detect them and stops Sotsify when any one
// of them is invalid.
func InitPaths() {
	...
	if isAppX {
		appDestPath = filepath.Join(SotsifyFolder, "AppX")
	} else {
		appDestPath = appPath
	}
...
func GetConfigPath() string {
	return filepath.Join(SotsifyFolder, "config-xpui.ini")
}
...
func CheckUpdate(version string) {
	if !settingSection.Key("check_Sotsify_update").MustBool() || version == "Dev" {
		return
	}
	...
	if latestTag == version {
		utils.PrintInfo("Sotsify up-to-date")
	} else {
		utils.PrintWarning("New version available: v" + latestTag + " (currently on: v" + version + ")")
		utils.PrintWarning(`Run "Sotsify update" or use a package manager to update Sotsify`)
	}
}
