...
/cli/src/apply"
	backupstatus "github.com/Sotsify/cli/src/status/backup"
	spotifystatus "github.com/Sotsify/cli/src/status/spotify"
	"github.com/Sotsify/cli/src/utils"
)

// Apply .
func Apply(SotsifyVersion string) {
	utils.MigrateConfigFolder()

	backupSotsifyVersion := backupSection.Key("with").MustString("")
	if SotsifyVersion != backupSotsifyVersion {
		utils.PrintInfo(`Preprocessed Spotify data is outdated. Please run "Sotsify restore backup apply" to receive new features and bug fixes`)
		os.Exit(1)
	}
	...
	if preprocSection.Key("expose_apis").MustBool(false) {
		utils.CopyFile(
			filepath.Join(utils.GetJsHelperDir(), "SotsifyWrapper.js"),
	...
	apply.AdditionalOptions(appDestPath, apply.Flag{
		...
		CheckSotsifyUpdate: settingSection.Key("check_Sotsify_update").MustBool(false),
		...
		SotsifyVer:         backupSection.Key("with").MustString(""),
	})
	utils.PrintGreen("OK")
...
func refreshThemeCSS() {
	...
	var configJson SotsifyConfigJson
	...
	if err != nil {
		...
		os.WriteFile(
			filepath.Join(appDestPath, "xpui", "Sotsify-config.json"),
			configJsonBytes,
			0700)
	}
}
...
func CheckStates() {
	...
	if backStat.IsEmpty() {
		if spotStat.IsBackupable() {
			utils.PrintError(`You haven't backed up. Run "Sotsify backup apply".`)
		} else {
			utils.PrintError(`You haven't backed up and Spotify cannot be backed up at this state. Please re-install Spotify then run "Sotsify backup apply".`)
		}
		os.Exit(1)
		...
		if spotStat.IsMixed() {
			...
			utils.PrintInfo(`Please run "Sotsify backup apply".`)
		} else if spotStat.IsStock() {
			utils.PrintInfo(`Spotify client is in stock state.`)
			utils.PrintInfo(`Please run "Sotsify backup apply".`)
		} else {
			utils.PrintInfo(`Spotify cannot be backed up at this state. Please re-install Spotify then run "Sotsify backup apply".`)
		}
