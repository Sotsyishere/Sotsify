...
import (
	...
	spotifystatus "github.com/


/cli/src/status/spotify"

	"github.com/Sotsify/cli/src/backup"
	"github.com/Sotsify/cli/src/preprocess"
...
func Backup(SotsifyVersion string) {
	if isAppX {
		utils.PrintInfo(`You are using Spotify Windows Store version, which is only partly supported.
Stop using Sotsify with Windows Store version unless you absolutely CANNOT install normal Spotify from installer.
Modded Spotify cannot be launched using original Shortcut/Start menu tile. To correctly launch Spotify with modification, please make a desktop shortcut that execute "Sotsify auto". After that, you can change its icon, pin to start menu or put in startup folder.`)
	...
	if !backStat.IsEmpty() {
		...
		if spotStat.IsBackupable() {
			...
			utils.PrintInfo(`Please restore first then backup, run "Sotsify restore backup" or re-install Spotify then run "Sotsify backup".`)
			os.Exit(1)
		}
	}
	...
	backupSection.Key("with").SetValue(SotsifyVersion)
	cfg.Write()
...
// Restore uses backup to revert every changes made by Sotsify.
func Restore() {
	CheckStates()
