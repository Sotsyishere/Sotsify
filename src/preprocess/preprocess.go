...
type Flag struct {
	...
	RemoveRTL bool
	// ExposeAPIs leaks some Spotify's API, functions, objects to Sotsify global object.
	ExposeAPIs bool
}

func readRemoteCssMap(tag string, cssTranslationMap *map[string]string) error {
	var cssMapURL string = "https://raw.githubusercontent.com/Sotsify/cli/" + tag + "/css-map.json"
	...
	filepath.Walk(appPath, func(path string, info os.FileInfo, err error) error {
		...
		case ".html":
			utils.ModifyFile(path, func(content string) string {
				...
				if flags.ExposeAPIs {
					tags += "<script src='helper/SotsifyWrapper.js'></script>\n"
					tags += "<!-- Sotsify helpers -->\n"
				}
...
func exposeAPIs_main(input string) string {
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf(`;globalThis.Sotsify.showNotification=(message,isError=false,msTimeout)=>%s({message,feedbackType:isError?"ERROR":"NOTICE",msTimeout});const %s=%s`, submatches[1], submatches[1], submatches[2])
		})
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf("%sSotsify._platform=%s", submatches[1], submatches[2])
		})
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf("%sSotsify.Platform.ReduxStore=%s", submatches[1], submatches[2])
		})
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf("%sSotsify.ReactComponent.PlatformProvider=%s", submatches[1], submatches[2])
		})
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf("%sSotsify.GraphQL.Definitions[%s]=%s", submatches[1], submatches[3], submatches[2])
		})
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf(`=Sotsify.GraphQL.Definitions["%s"]%s`, submatches[2], submatches[1])
		})
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf("%sSotsify.Snackbar.enqueueCustomSnackbar=%s;", submatches[0], submatches[1])
		})
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf("=Sotsify.Snackbar.enqueueCustomSnackbar%s;", submatches[0])
		})
	...
	utils.Replace(
		...
		`\(\({[^}]*,\s*imageSrc`,
		func(submatches ...string) string {
			return fmt.Sprintf("Sotsify.Snackbar.enqueueImageSnackbar=%s", submatches[0])
		})

	// Menu hook
	utils.Replace(&input, `("Menu".+?children:)([\w$][\w$\d]*)`, func(submatches ...string) string {
		return fmt.Sprintf("%s[Sotsify.ContextMenuV2.renderItems(),%s].flat()", submatches[1], submatches[2])
	})
	...
	utils.Replace(&input, `\(0,([\w_$]+)\.jsx\)\([\w_$]+\.[\w_$]+,\{value:"contextmenu"[^\}]+\}\)\}\)`, func(submatches ...string) string {
		return fmt.Sprintf("(0,%s.jsx)((Sotsify.ContextMenuV2._context||(Sotsify.ContextMenuV2._context=%s.createContext(null))).Provider,{value:{props:%s?.props,trigger:%s,target:%s},children:%s})", submatches[1], react, menu, trigger, target, submatches[0])
	})
...
func exposeAPIs_vendor(input string) string {
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf(`,(globalThis.Sotsify.URI=%s)%s`, submatches[1], submatches[0])
		})

	// URI after 1.2.4
	if !strings.Contains(input, "Sotsify.URI") {
		...
		if len(URIObj) != 0 {
			...
			input = strings.Replace(
				input,
				URI,
				fmt.Sprintf("%s;Sotsify.URI=%s;", URI, URIObj[1]),
				1)
		}
	}
	...
	utils.Replace(
		...
		`(\w+ [\w$_]+)=[\w$_]+\([\w$_]+>>>0\)`,
		func(submatches ...string) string {
			return fmt.Sprintf("%s=Sotsify._getStyledClassName(arguments,this)", submatches[1])
		})
	...
	utils.Replace(
		...
		`([\w\$_]+)\.setDefaultProps=`,
		func(submatches ...string) string {
			return fmt.Sprintf("Sotsify.Tippy=%s;%s", submatches[1], submatches[0])
		})
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf("%s=Sotsify.ReactFlipToolkit.spring=%s", submatches[1], submatches[2])
		})
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf("Sotsify.Snackbar=%s", submatches[0])
		})
...
func FetchLatestTagMatchingVersion(version string) (string, error) {
	...
	res, err := http.Get("https://api.github.com/repos/Sotsify/cli/releases")
	if err != nil {
		return "", err
	}
