...
(async function shufflePlus() {
	if (!(Sotsify.CosmosAsync && Sotsify.Platform)) {
	...
	function getConfig() {
		try {
			const parsed = JSON.parse(Sotsify.LocalStorage.get("shufflePlus:settings"));
			if (parsed && typeof parsed === "object") {
				return parsed;
			}
			throw "";
		} catch {
			Sotsify.LocalStorage.set("shufflePlus:settings", "{}");
	...
	function saveConfig() {
		Sotsify.LocalStorage.set("shufflePlus:settings", JSON.stringify(CONFIG));
	}
			...
			return React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(DisplayIcon, { icon: Sotsify.SVGIcons.check, size: 16 })
					)
				)
			);
		}
	...
	new Sotsify.Menu.Item("Shuffle+", false, settingsPage, "shuffle").register();

	const { Type } = Sotsify.URI;

	function shouldAddShufflePlus(uri) {
		if (uri.length === 1) {
			const uriObj = Sotsify.URI.fromString(uri[0]);
			switch (uriObj.type) {
				case Type.PLAYLIST:
	...
	function shouldAddShufflePlusLiked(uri) {
		const uriObj = Sotsify.URI.fromString(uri[0]);
		if (Sotsify.Platform.History.location.pathname === "/collection/tracks") {
			switch (uriObj.type) {
				case Type.TRACK:
					return true;
			}
	...
	function shouldAddShufflePlusLocal(uri) {
		const uriObj = Sotsify.URI.fromString(uri[0]);
		if (Sotsify.Platform.History.location.pathname === "/collection/local-files") {
			switch (uriObj.type) {
				case Type.TRACK:
	...
	function renderQueuePlaybarButton() {
		if (!playbarButton) {
			playbarButton = new Sotsify.Playbar.Button(
	...
	async function fetchPlaylistTracks(uri) {
		const res = await Sotsify.CosmosAsync.get(`sp://core-playlist/v1/playlist/spotify:playlist:${uri}/rows`, {
	...
	async function fetchFolderTracks(uri) {
		const res = await Sotsify.CosmosAsync.get("sp://core-playlist/v1/rootlist", {
	...
	async function fetchAlbumTracks(uri, includeMetadata = false) {
		const { queryAlbumTracks } = Sotsify.GraphQL.Definitions;
		const { data, errors } = await Sotsify.GraphQL.Request(queryAlbumTracks, { uri, offset: 0, limit: 100 });
	...
	async function scanForTracksFromAlbums(res, artistName, type) {
		...
		for (const album of res) {
			...
			Sotsify.showNotification(`${artistFetchTypeCount[type]} / ${res.length} ${type}s`);
	...
	async function fetchArtistTracks(uri) {
		const { queryArtistDiscographyAll } = Sotsify.GraphQL.Definitions;
		...
		const discography = await Sotsify.GraphQL.Request(queryArtistDiscographyAll, {
			uri,
			offset: 0,
		...
		const overview = await Sotsify.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Sotsify.Locale.getLocale(),
	...
	async function fetchArtistLikedTracks(uri) {
		const artistRes = await Sotsify.CosmosAsync.get(`sp://core-collection/unstable/@/list/tracks/artist/${uri}?responseFormat=protobufJson`);
	...
	async function fetchArtistTopTenTracks(uri) {
		const { queryArtistOverview } = Sotsify.GraphQL.Definitions;
		const { data, errors } = await Sotsify.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Sotsify.Locale.getLocale(),
			includePrerelease: false,
		});
	...
	async function fetchLikedTracks() {
		const res = await Sotsify.CosmosAsync.get("sp://core-collection/unstable/@/list/tracks/all?responseFormat=protobufJson");
	...
	async function fetchLocalTracks() {
		const res = await Sotsify.Platform.LocalFilesAPI.getTracks();

		return res.map((track) => track.uri);
	}

	function fetchQueue() {
		const { _queueState } = Sotsify.Platform.PlayerAPI._queue;
	...
	async function fetchCollection(uriObj) {
		...
		const { pathname } = Sotsify.Platform.History.location;
	...
	async function fetchShows(uri) {
		const res = await Sotsify.CosmosAsync.get(`sp://core-show/v1/shows/${uri}?responseFormat=protobufJson`);
	...
	async function Queue(list, context, type) {
		...
		const { _queue, _client } = Sotsify.Platform.PlayerAPI._queue;
		...
		if (context) {
			const { sessionId } = Sotsify.Platform.PlayerAPI.getState();
			Sotsify.Platform.PlayerAPI.updateContext(sessionId, { uri: context, url: `context://${context}` });
		}
		...
		switch (type) {
			case Type.ARTIST:
				if (CONFIG.artistMode === "topTen") {
					Sotsify.showNotification(`Shuffled Top ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "likedSongArtist") {
					Sotsify.showNotification(`Shuffled ${count} Liked Songs`);
					break;
				}
				if (CONFIG.artistMode === "single") {
					Sotsify.showNotification(`Shuffled ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "album") {
					Sotsify.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, Total of ${count} Songs`);
					break;
				}
				Sotsify.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
				break;
			default:
				Sotsify.showNotification(`Shuffled ${count} Songs`);
		}
	...
	async function fetchAndPlay(rawUri) {
		...
		try {
			if (rawUri === "queue") {
				...
				const uriObj = Sotsify.URI.fromString(rawUri);
				type = uriObj.type;
				...
				if (!list?.length) {
					Sotsify.showNotification("Nothing to play", true);
					return;
				}
			...
			Sotsify.showNotification(String(error), true);
			console.error(error);
		}
	}
})();
