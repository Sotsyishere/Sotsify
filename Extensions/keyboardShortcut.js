...
(function KeyboardShortcut() {
	...
	const binds = {
		...
		"ctrl+q": {
			callback: () =>
				Sotsify.CosmosAsync.post("sp://esperanto/spotify.desktop.lifecycle_esperanto.proto.DesktopLifecycle/Shutdown") &&
				Sotsify.CosmosAsync.post("sp://desktop/v1/shutdown"),
		},
		...
		"shift+h": { callback: () => Sotsify.Platform.History.goBack() },
		"shift+l": { callback: () => Sotsify.Platform.History.goForward() },

		// M to Like/Unlike track
		m: { callback: () => Sotsify.Player.toggleHeart() },

		// Forward Slash to open search page
		"/": { callback: () => Sotsify.Platform.History.replace("/search") },

		// CTRL + Arrow Left Next and CTRL + Arrow Right  Previous Song
		"ctrl+left": { callback: () => Sotsify.Player.prev() },
		"ctrl+right": { callback: () => Sotsify.Player.next() },

		// CTRL + Arrow Up Increase Volume CTRL + Arrow Down Decrease Volume
		"ctrl+up": { callback: () => Sotsify.Player.setVolume(Sotsify.Player.getVolume() + 0.05) },
		"ctrl+down": { callback: () => Sotsify.Player.setVolume(Sotsify.Player.getVolume() - 0.05) },
	...
	for (const [key, { staticCondition, callback }] of Object.entries(binds)) {
		if (typeof staticCondition === "undefined" || staticCondition) {
			Sotsify.Mousetrap.bind(key, (event) => {
	...
	function findActiveIndex(allItems) {
		...
		const historyURI = Sotsify.Platform.History.location.pathname.replace(/^\//, "spotify:").replace(/\//g, ":");
	...
	function rotateSidebar(direction) {
		const allItems = document.querySelectorAll(
			"#Sotsify-sticky-list .main-yourLibraryX-navLink, .main-yourLibraryX-listItem > div:not(:has([data-skip-in-keyboard-nav])) > div:first-child"
		);
...
.vim-key {
	...
	const mousetrap = new Sotsify.Mousetrap(document);
	...
	/**
	 *
	 * @param {Sotsify.Keyboard.ValidKey} key
	 */
	this.setCancelKey = function (key) {
		mousetrap.bind(Sotsify.Keyboard.KEYS[key], this.deactivate.bind(this));
	};
