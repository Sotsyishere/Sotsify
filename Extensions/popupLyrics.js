...
function PopupLyrics() {
	const { Player, CosmosAsync, LocalStorage, ContextMenu } = Sotsify;
	...
	const LyricProviders = {
		...
		async fetchLrclib(info) {
			...
			const body = await fetch(finalURL, {
				headers: {
					"x-user-agent": `Sotsify v${Sotsify.Config.version} (https://github.com/Sotsify/cli)`,
				},
			});
	...
	const userConfigs = {
		...
		services: {
			...
			musixmatch: {
				...
				desc: `Fully compatible with Spotify. Requires a token that can be retrieved from the official Musixmatch app. Follow instructions on <a href="https://Sotsify.app/docs/faq#sometimes-popup-lyrics-andor-lyrics-plus-seem-to-not-work">Sotsify Docs</a>.`,
	...
	const button = new Sotsify.Topbar.Button("Popup Lyrics", "lyrics", () => {
		if (!lyricVideoIsOpen) {
	...
	async function updateTrack() {
		...
		if (!Sotsify.URI.isTrack(Player.data.item.uri) && !Sotsify.URI.isLocalTrack(Player.data.item.uri)) {
			return;
		}
    ...
    <div class="col action">
        <button class="switch small">
            <svg height="10" width="10" viewBox="0 0 16 16" fill="currentColor">
                ${Sotsify.SVGIcons["chart-up"]}
            </svg>
        </button>
        <button class="switch small">
            <svg height="10" width="10" viewBox="0 0 16 16" fill="currentColor">
                ${Sotsify.SVGIcons["chart-down"]}
            </svg>
        </button>
        <button class="switch">
            <svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor">
