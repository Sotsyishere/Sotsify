...
`;
	...
	let debouncing = 0;
	Sotsify.Player.addEventListener("onprogress", (event) => {
		if (start != null && end != null) {
			if (debouncing) {
			...
			const percent = Sotsify.Player.getProgressPercent();
			if (percent > end || percent < start) {
				debouncing = event.timeStamp;
				Sotsify.Player.seek(start);
				return;
			}
		}
	});

	Sotsify.Player.addEventListener("songchange", reset);
