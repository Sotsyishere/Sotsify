...
(function Bookmark() {
	const { CosmosAsync, Player, LocalStorage, ContextMenu, URI } = 
	...
	// Local Storage keys
	const STORAGE_KEY = "bookmark_Sotsify";
...
`;
			...
			Sotsify.Tippy(this.querySelectorAll("[data-tippy-content]"), Sotsify.TippyProps);
			if (isPlayable) {
	...
	function createMenuItem(title, callback) {
		...
		Sotsify.ReactDOM.render(
			Sotsify.React.createElement(
				Sotsify.ReactComponent.MenuItem,
				{
					onClick: () => {
						callback?.();
					},
	...
	async function storeThisPage() {
		...
		const context = Sotsify.Platform.History.location.pathname;
		try {
			contextUri = Sotsify.URI.fromString(context);
		} catch (e) {
			Sotsify.showNotification("Cannot bookmark this page", true);
			return;
		}
	...
	function getTrackMeta() {
		...
		const playerState = Sotsify.Player.data;
...
`;
	...
	async function onLinkClick(info) {
		if (info.context?.startsWith("/")) {
			Sotsify.Platform.History.push(info.context);
			return;
		}
		const url = Sotsify.URI.fromString(info.uri).toURLPath(true);
		Sotsify.Platform.History.push(url);
	}
	...
	function onPlayClick(info) {
		...
		Sotsify.Player.playUri(uri, {}, options);
	}

	const fetchAlbum = async (uri) => {
		const { getAlbum } = Sotsify.GraphQL.Definitions;
		const { data } = await Sotsify.GraphQL.Request(getAlbum, {
			uri,
	...
	const fetchArtist = async (uri) => {
		...
		const { data } = await Sotsify.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Sotsify.Locale.getLocale(),
	...
	const fetchTrack = async (uri, uid, context) => {
		...
		let newContext;
		if (context && uid && Sotsify.URI.isPlaylistV1OrV2(context)) {
			newContext = `${Sotsify.URI.fromString(context).toURLPath(true)}?uid=${uid}`;
		}
		return {
			uri,
			title: res.name,
	...
	const fetchPlaylist = async (uri) => {
		const res = await Sotsify.CosmosAsync.get(`sp://core-playlist/v1/playlist/${uri}/metadata`, {
	...
	new Sotsify.ContextMenu.Item(
		...
		async ([uri], [uid] = [], context = undefined) => {
			...
			let meta;
			switch (type) {
				case Sotsify.URI.Type.TRACK:
					meta = await fetchTrack(uri, uid, context);
					break;
				case Sotsify.URI.Type.ALBUM:
					meta = await fetchAlbum(uri);
					break;
				case Sotsify.URI.Type.ARTIST:
					meta = await fetchArtist(uri);
					break;
				case Sotsify.URI.Type.SHOW:
					meta = await fetchShow(uri);
					break;
				case Sotsify.URI.Type.EPISODE:
					meta = await fetchEpisode(uri);
					break;
				case Sotsify.URI.Type.PLAYLIST:
				case Sotsify.URI.Type.PLAYLIST_V2:
