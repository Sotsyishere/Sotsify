...
(function TrashBin() {
	...
	if (!Sotsify.Player.data || !Sotsify.LocalStorage || !skipBackBtn) {
	...
	function createSlider(name, desc, defaultVal, callback) {
		...
		slider.onclick = () => {
			...
			Sotsify.LocalStorage.set(name, state);
	...
	function settingsContent() {
		...
		content.appendChild(
			createButton("Clear ", "Clear all items from trashbin (cannot be reverted).", () => {
				...
				putDataLocal();
				Sotsify.showNotification("Trashbin cleared!");
			})
		);
	}
	...
	function initValue(item, defaultValue) {
		try {
			const value = JSON.parse(Sotsify.LocalStorage.get(item));
	...
	const widget = new Sotsify.Playbar.Widget(
		THROW_TEXT,
		trashbinIcon,
		(self) => {
			const uri = Sotsify.Player.data.item.uri;
			const uriObj = Sotsify.URI.fromString(uri);
			const type = uriObj.type;

			if (!trashSongList[uri]) {
				trashSongList[uri] = true;
				if (shouldSkipCurrentTrack(uri, type)) Sotsify.Player.next();
				Sotsify.showNotification("Song added to trashbin");
			} else {
				delete trashSongList[uri];
				setWidgetState(false);
				Sotsify.showNotification("Song removed from trashbin");
			}
	...
	setWidgetState(
		trashSongList[Sotsify.Player.data.item.uri],
		Sotsify.URI.fromString(Sotsify.Player.data.item.uri).type !== Sotsify.URI.Type.TRACK
	);
	...
	function refreshEventListeners(state) {
		...
		if (state) {
			...
			Sotsify.Player.addEventListener("songchange", watchChange);
			enableWidget && widget.register();
			watchChange();
		} else {
			skipBackBtn.removeEventListener("click", eventListener);
			Sotsify.Player.removeEventListener("songchange", watchChange);
			widget.deregister();
		}
	}
	...
	function watchChange() {
		...
		setWidgetState(isBanned, Sotsify.URI.fromString(data.item.uri).type !== Sotsify.URI.Type.TRACK);
	...
	function shouldSkipCurrentTrack(uri, type) {
		const curTrack = Sotsify.Player.data.item;
		if (type === Sotsify.URI.Type.TRACK) {
			if (uri === curTrack.uri) {
				return true;
			}
		}

		if (type === Sotsify.URI.Type.ARTIST) {
			let count = 1;
	...
	function toggleThrow(uris) {
		...
		const list = type === Sotsify.URI.Type.TRACK ? trashSongList : trashArtistList;

		if (!list[uri]) {
			list[uri] = true;
			if (shouldSkipCurrentTrack(uri, type)) Sotsify.Player.next();
			Sotsify.Player.data?.item.uri === uri && setWidgetState(true);
			Sotsify.showNotification(type === Sotsify.URI.Type.TRACK ? "Song added to trashbin" : "Artist added to trashbin");
		} else {
			delete list[uri];
			Sotsify.Player.data?.item.uri === uri && setWidgetState(false);
			Sotsify.showNotification(type === Sotsify.URI.Type.TRACK ? "Song removed from trashbin" : "Artist removed from trashbin");
		}
	...
	function shouldAddContextMenu(uris) {
		...
		const uri = uris[0];
		const uriObj = Sotsify.URI.fromString(uri);
		if (uriObj.type === Sotsify.URI.Type.TRACK) {
			this.name = trashSongList[uri] ? UNTHROW_TEXT : THROW_TEXT;
			return true;
		}

		if (uriObj.type === Sotsify.URI.Type.ARTIST) {
	...
	const cntxMenu = new Sotsify.ContextMenu.Item(THROW_TEXT, toggleThrow, shouldAddContextMenu, trashbinIcon);
	cntxMenu.register();

	function putDataLocal() {
		Sotsify.LocalStorage.set("TrashSongList", JSON.stringify(trashSongList));
		Sotsify.LocalStorage.set("TrashArtistList", JSON.stringify(trashArtistList));
	}
	...
	function exportItems() {
		...
		Sotsify.Platform.ClipboardAPI.copy(JSON.stringify(data));
		Sotsify.showNotification("Copied to clipboard");
	}
		...
		input.onchange = (e) => {
			...
			reader.onload = (e) => {
				try {
					...
					putDataLocal();
					Sotsify.showNotification("File Import Successful!");
				} catch (e) {
					Sotsify.showNotification("File Import Failed!", true);
					console.error(e);
				}
			};
			reader.onerror = () => {
				Sotsify.showNotification("File Read Failed!", true);
