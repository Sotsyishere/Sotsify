...
(function FullAppDisplay() {
	if (!Sotsify.Keyboard || !Sotsify.React || !Sotsify.ReactDOM) {
		setTimeout(FullAppDisplay, 200);
		return;
	}

	const { React: react, ReactDOM: reactDOM } = Sotsify;
...
#fad-art {
	...
	const ProgressBar = () => {
		const [progress, setProgress] = useState(Sotsify.Player.getProgress());
		const duration = Sotsify.Platform.PlayerAPI._state.duration;
		...
		useEffect(() => {
			...
			Sotsify.Player.addEventListener("onprogress", update);
			return () => Sotsify.Player.removeEventListener("onprogress", update);
		}, [isDragging]);
		...
		const handleClick = (e) => {
			...
			Sotsify.Player.seek(newProgress);
		...
		return react.createElement(
			...
			react.createElement("span", { id: "fad-elapsed" }, Sotsify.Player.formatTime(progress)),
			react.createElement(
				"div",
				{
					id: "fad-progress",
			...
			react.createElement("span", { id: "fad-duration" }, Sotsify.Player.formatTime(duration))
		);
	};

	const PlayerControls = () => {
		const [value, setValue] = useState(Sotsify.Player.isPlaying());
		useEffect(() => {
			const update = ({ data }) => setValue(!data.isPaused);
			Sotsify.Player.addEventListener("onplaypause", update);
			return () => Sotsify.Player.removeEventListener("onplaypause", update);
		});
		return react.createElement(
			"div",
			{ id: "fad-controls" },
			react.createElement(ButtonIcon, {
				icon: Sotsify.SVGIcons["skip-back"],
				onClick: Sotsify.Player.back,
			}),
			react.createElement(ButtonIcon, {
				icon: Sotsify.SVGIcons[value ? "pause" : "play"],
				onClick: Sotsify.Player.togglePlay,
			}),
			react.createElement(ButtonIcon, {
				icon: Sotsify.SVGIcons["skip-forward"],
				onClick: Sotsify.Player.next,
			})
		);
	};
	...
	class FAD extends react.Component {
		constructor(props) {
			...
			this.state = {
				...
				releaseDate: "",
				cover: "",
				heart: Sotsify.Player.getHeart(),
			};
			this.currTrackImg = new Image();
			this.nextTrackImg = new Image();
			this.mousetrap = new Sotsify.Mousetrap();
		}

		async getAlbumDate(uri) {
			const id = uri.replace("spotify:album:", "");

			const albumInfo = await Sotsify.CosmosAsync.get(`https://api.spotify.com/v1/albums/${id}`);
		...
		async fetchInfo() {
			const meta = Sotsify.Player.data.item.metadata;
		...
		componentDidMount() {
			...
			Sotsify.Player.addEventListener("songchange", this.updateInfo);
			this.updateInfo();
			...
			Sotsify.Platform.PlayerAPI._events.addListener("queue_update", this.onQueueChange);
			this.mousetrap.bind("esc", deactivate);
			window.dispatchEvent(new Event("fad-request"));
		}

		componentWillUnmount() {
			Sotsify.Player.removeEventListener("songchange", this.updateInfo);
			Sotsify.Platform.PlayerAPI._events.removeListener("queue_update", this.onQueueChange);
					...
					react.createElement(
								...
								react.createElement(
									...
									react.createElement(
										...
										react.createElement(DisplayIcon, {
											icon: Sotsify.SVGIcons[this.state.heart ? "heart-active" : "heart"],
											size: 50,
										})
									)
								),
						...
						react.createElement(
							...
							react.createElement(SubInfo, {
								...
								icon: Sotsify.SVGIcons.artist,
							}),
							CONFIG.showAlbum &&
								react.createElement(SubInfo, {
									id: "fad-album",
									text: this.state.album,
									icon: Sotsify.SVGIcons.album,
								}),
							CONFIG.showReleaseDate &&
								react.createElement(SubInfo, {
									id: "fad-release-date",
									text: this.state.releaseDate,
									icon: Sotsify.SVGIcons.clock,
								}),
							react.createElement(
								"div",
								{
	...
	function deactivate() {
		...
		if (lastApp && lastApp !== "/lyrics-plus") {
			Sotsify.Platform.History.push(lastApp);
		}
	}
	...
	function checkLyricsPlus() {
		return Sotsify.Config?.custom_apps?.includes("lyrics-plus") || !!document.querySelector("a[href='/lyrics-plus']");
	}

	function requestLyricsPlus() {
		if (CONFIG.lyricsPlus && checkLyricsPlus()) {
			lastApp = Sotsify.Platform.History.location.pathname;
			if (lastApp !== "/lyrics-plus") {
				Sotsify.Platform.History.push("/lyrics-plus");
			}
		}
		window.dispatchEvent(new Event("fad-request"));
	}
	...
	function getConfig() {
		try {
			const parsed = JSON.parse(Sotsify.LocalStorage.get("full-app-display-config") || "{}");
			if (parsed && typeof parsed === "object") {
				return parsed;
			}
			throw "";
		} catch {
			Sotsify.LocalStorage.set("full-app-display-config", "{}");
			return {};
		}
	}

	function saveConfig() {
		Sotsify.LocalStorage.set("full-app-display-config", JSON.stringify(CONFIG));
	}
...
`,
	...
	new Sotsify.Topbar.Button(
		"Full App Display",
		`<svg role="img" height="16" width="16" viewBox="0 0 16 16" fill="currentColor">${Sotsify.SVGIcons.projector}</svg>`,
		activate
	);

	Sotsify.Mousetrap.bind("f11", toggleFad);
})();
